...
rm -rf hddcoin-light-gui/build || true
rm -rf hddcoin-light-gui/daemon || true
rm -rf hddcoin-light-gui/node_modules || true
rm hddcoin-light-gui/temp.json || true
( cd "$PWD/hddcoin-light-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
