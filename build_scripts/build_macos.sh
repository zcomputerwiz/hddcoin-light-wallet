...
electron-packager . "HDDcoin Light Wallet" --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" == true ]; then
  electron-osx-sign HDDcoin\ Light\ Wallet-darwin-x64/HDDcoin\ Light\ Wallet.app --platform=darwin \
...
mv HDDcoin\ Light\ Wallet-darwin-x64 ../../../build_scripts/dist/
cd ../../../build_scripts || exit

DMG_NAME="HDDcoin-Wallet-$HDDCOIN_INSTALLER_VERSION.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/HDDcoin\ Light\ Wallet-darwin-x64/HDDcoin\ Light\ Wallet.app HDDcoin-Wallet-$HDDCOIN_INSTALLER_VERSION \
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.hddcoin.wallet \
...
# xcrun altool --notarize-app -f HDDcoin-0.1.X.dmg --primary-bundle-id net.hddcoin.wallet -u username -p password
