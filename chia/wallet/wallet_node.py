...
from hddcoin.full_node.weight_proof import chunks
...
from hddcoin.types.weight_proof import WeightProof, SubEpochData
from hddcoin.util.byte_types import hexstr_to_bytes
from hddcoin.util.ints import uint32, uint64
from hddcoin.util.keychain import KeyringIsLocked
...
class WalletNode:
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSHDDcoinConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    def on_disconnect(self, peer: WSHDDcoinConnection):
    ...
    async def trusted_sync(self, full_node: WSHDDcoinConnection):
        """
    ...
    async def subscribe_to_phs(self, puzzle_hashes: List[bytes32], peer: WSHDDcoinConnection, height=uint32(0)):
        """
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSHDDcoinConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSHDDcoinConnection) -> Optional[HeaderBlock]:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSHDDcoinConnection, peak: HeaderBlock
    ...
    async def untrusted_sync_to_peer(
        self, peer: WSHDDcoinConnection, weight_proof: WeightProof, syncing: bool, fork_height: int
    ):
        assert self.wallet_state_manager is not None
