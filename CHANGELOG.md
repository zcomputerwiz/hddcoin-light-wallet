...
We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Passphrase-Protected-HDDcoin-Keys-and-Key-Storage-Migration). We also launched a new HDDcoinlisp compiler in clvm_tools_rs which substantially improves compile time for HDDcoinlisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.
...
- Enabled experimental use of a new HDDcoinlisp compiler in clvm_tools_rs in hddcoin-blockchain, which is off by default, and substantially improves compile time.
...
- Beta builds are built on every merge to main, and are now available from <https://hddcoin.net/download/>.
...
- This is a hard fork/breaking change from RC6/7. THDD Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.hddcoin.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- We have moved to a new and much higher scalability download.hddcoin.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.hddcoin.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.hddcoin.net/latest/Setup-MacOS.dmg).
- HDDcoin wheels not on Pypi and some dependecies not found there also are now on pypi.chia.net.
...
- PRs and commits to the hddcoin-light-gui repository will automatically have their locales updated.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.hddcoin.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.hddcoin.net via https, for example.
...
- There is now remote UI support. [Documents](https://github.com/HDDcoin-Network/hddcoin-light-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- The GUI is now in a separate submodule repository from hddcoin-blockchain, [hddcoin-light-gui](https://github.com/HDDcoin-Network/hddcoin-light-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `hddcoin-light-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [hddcoin.net](https://hddcoin.net/). Among the improvements this gives the HDDcoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from HDDcoin Network at backup.hddcoin.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the HDDcoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.hddcoin.net, your own installation, or a third party's version of it.
...
- More details on the release at [https://www.hddcoin.net/developer/](https://www.hddcoin.net/developer/)
